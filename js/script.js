...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const spanishUnits = [
    // Unit 1
    [
      {front:"Hola", back:"Hello"}, {front:"Adiós", back:"Goodbye"},
      {front:"Gracias", back:"Thank you"}, {front:"Por favor", back:"Please"},
      {front:"Sí", back:"Yes"}, {front:"No", back:"No"},
      {front:"¿Cómo estás?", back:"How are you?"}, {front:"Bien", back:"Good"},
      {front:"Mal", back:"Bad"}, {front:"¿Qué tal?", back:"What's up?"},
      {front:"Buenos días", back:"Good morning"}, {front:"Buenas noches", back:"Good night"},
      {front:"Perdón", back:"Sorry"}, {front:"Lo siento", back:"I'm sorry"},
      {front:"Salud", back:"Bless you"}, {front:"Mucho gusto", back:"Nice to meet you"},
      {front:"¿Dónde está?", back:"Where is?"}, {front:"Hoy", back:"Today"},
      {front:"Mañana", back:"Tomorrow"}, {front:"Ayer", back:"Yesterday"}
    ],
    // Unit 2
    [
      {front:"Amigo", back:"Friend"}, {front:"Familia", back:"Family"},
      {front:"Escuela", back:"School"}, {front:"Casa", back:"House"},
      {front:"Comida", back:"Food"}, {front:"Agua", back:"Water"},
      {front:"Libro", back:"Book"}, {front:"Lapiz", back:"Pencil"},
      {front:"Profesor", back:"Teacher"}, {front:"Estudiante", back:"Student"},
      {front:"Perro", back:"Dog"}, {front:"Gato", back:"Cat"},
      {front:"Grande", back:"Big"}, {front:"Pequeño", back:"Small"},
      {front:"Nuevo", back:"New"}, {front:"Viejo", back:"Old"},
      {front:"Rápido", back:"Fast"}, {front:"Lento", back:"Slow"},
      {front:"Bonito", back:"Pretty"}, {front:"Feo", back:"Ugly"}
    ],
    // Units 3–5 can follow similarly...
  ];
  ...
  function loadUnit(unitIndex){
    ...
    showCard();
    showPractice();
    loadUnitTest(unitIndex);
  }

  function showCard(){
    const cardBox = document.getElementById("flashcardBox");
    if(!cardBox) return;
    cardBox.textContent = flipped ? spanishUnits[currentUnit][currentCard].back : spanishUnits[currentUnit][currentCard].front;
  }
  ...
  window.checkPractice = function(){
    ...
    const answer = spanishUnits[currentUnit][currentCard].front.toLowerCase();
    if(input === answer) result.textContent="✅ Correct!";
  ...
  window.gradeTest = function(){
    let score = 0;
    spanishUnits[currentUnit].forEach((card,i)=>{
      const val = document.getElementById(`testInput${i}`).value.toLowerCase();
      if(val === card.front.toLowerCase()) score++;
    });
    document.getElementById("testResult").textContent = `Score: ${score}/${spanishUnits[currentUnit].length}`;
  }

  loadUnit(0); // Load Unit 1 by default
});
